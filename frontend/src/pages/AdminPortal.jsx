...
function ProductForm({ initial, onClose, onSaved }) {
  ...
  return <form onSubmit={submit} className="grid sm:grid-cols-2 gap-5">
    ...
    <div><label className={label}>Category</label><select className={field} value={form.category} onChange={(e) => set("category", e.target.value)}><option value="clothing">Clothing</option><option value="cosmetics">Cosmetics</option><option value="accessories">Accessories</option><option value="service">Service</option></select></div>
...
export default function AdminPortal() {
  ...
  const nav = [{ id: "overview", text: "Overview", icon: BarChart3 }, { id: "products", text: "Products & services", icon: Package }, { id: "specials", text: "Specials", icon: Gift }, { id: "orders", text: "Orders", icon: ShoppingBag }, { id: "bookings", text: "Bookings", icon: CalendarDays }, { id: "settings", text: "Settings", icon: Settings }];
  return <div className="min-h-screen pt-24 bg-[#f5f0e8] flex flex-col lg:flex-row">
    ...
    <section className="flex-1 p-5 sm:p-10 overflow-hidden"><div className="max-w-6xl mx-auto">{error && <div className="bg-red-50 text-red-700 p-4 mb-5">{error}</div>}
      ...
      {tab === "products" && <><div className="flex justify-between items-end mb-6"><div><p className="overline text-terracotta">Catalogue</p><h2 className="font-serif text-4xl">Products & services</h2></div><button onClick={() => setModal({ type: "product" })} className="bg-espresso text-white px-5 py-3">+ Add new</button></div><div className="bg-white overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="p-4">Item</th><th className="p-4">Category</th><th className="p-4">Price</th><th className="p-4">Stock</th><th className="p-4">Status</th><th className="p-4"></th></tr></thead><tbody>{data.products.map(p=><tr key={p.id} className="border-b border-espresso/5"><td className="p-4"><div className="flex gap-3 items-center"><img src={p.image} alt="" className="w-12 h-14 object-cover bg-sand"/><span>{p.name}</span></div></td><td className="p-4 capitalize">{p.category}</td><td className="p-4">{formatBWP(p.price)}</td><td className="p-4">{p.stock}</td><td className="p-4">{p.active === false ? "Hidden" : "Live"}</td><td className="p-4 whitespace-nowrap"><button onClick={() => setModal({ type:"product", item:p })} className="underline mr-4">Edit</button><button onClick={() => remove("products",p.id)} className="text-destructive"><Trash2 size={16}/></button></td></tr>)}</tbody></table></div></>}
      ...
      {tab === "bookings" && <AdminRecords items={data.bookings} type="bookings" status={status} />}
    ...
    {modal?.type === "product" && <Modal title={modal.item ? "Edit item" : "Add item"} onClose={()=>setModal(null)}><ProductForm initial={modal.item} onClose={()=>setModal(null)} onSaved={()=>{setModal(null);load();}}/></Modal>}
...
function AdminRecords({ items, type, status }) {
  const options = type === "orders" ? ["pending_payment","paid","processing","ready","completed","cancelled"] : ["new","confirmed","completed","cancelled"];
  return <><p className="overline text-terracotta">Management</p><h2 className="font-serif text-4xl mb-6 capitalize">{type}</h2><div className="space-y-4">{items.length===0 && <div className="bg-white p-8 text-muted-foreground">No {type} yet.</div>}{items.map(x=><div key={x.id} className="bg-white border border-espresso/5 p-5"><div className="flex flex-col md:flex-row justify-between gap-5"><div><p className="overline text-terracotta">{x.order_number || x.booking_number}</p><h3 className="font-serif text-2xl mt-1">{x.shipping?.full_name || x.customer_name}</h3><p className="text-sm text-muted-foreground mt-1">{x.shipping?.phone || x.phone} · {new Date(x.created_at).toLocaleString()}</p>{x.service_name && <p className="mt-3">{x.service_name} — {x.preferred_date} at {x.preferred_time}</p>}{x.items && <p className="mt-3">{x.items.map(i=>`${i.name} ×${i.quantity}`).join(", ")} · <strong>{formatBWP(x.total)}</strong></p>}</div><select className={`${field} md:w-48 self-start`} value={x.status} onChange={e=>status(type,x.id,e.target.value)}>{options.map(o=><option key={o} value={o}>{o.replaceAll("_"," ")}</option>)}</select></div></div>)}</div></>;
}
